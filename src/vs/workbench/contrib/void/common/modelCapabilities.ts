...
export const defaultModelsOfProvider = {
	...
	liteLLM: [],
	divisionAPI: [
		// 'division-orchestrator' のみハードコード保持。これは Division Project (マルチエージェント
		// オーケストレーション) を表す合成エントリなので、`/api/models` には含まれない可能性があるが
		// UI の選択既定値として常に必要。 単一モデル (gpt-4 / claude-* / gemini-* など) は
		// `refreshModelService.ts` が `/api/models` を 5 秒間隔でポーリングして自動同期するので、
		// ここでは列挙しない (古いハードコード一覧が dropdown を汚す問題を防ぐ)。
