...
const saveFlowResultAsMd = (
	...
	const mdContent = `# ${filename.replace('.md', '')} — ${title}\n\n> **Generated by Division API**  \n> Session: \`${sessionId}\`  \n> Role: \`${role}\`  \n> Date: ${now}\n\n---\n\n${content}\n`;
		...
		if (workspaceFolderPath) {
			...
			if (pendingState) {
				...
				if (lastMsgContent.includes('[FLOW_REVIEW_APPROVED]')) {
					// Resume orchestration from where we left off
					...
					appendText(`## ▶️ オーケストレーション再開\n\n`);
					appendText(`📎 **Session:** \`${sessionId}\`\n`);
					appendText(`📦 **残りタスク:** ${tasks.length - startIndex}/${tasks.length}\n\n`);
					appendText(`---\n\n## 🚀 Phase 3: Task Execution (続行)\n\n`);

					// Execute remaining tasks (reuse the same task loop logic below)
					for (let i = startIndex; i < tasks.length; i++) {
						...
						// Build task prompt — for coding tasks, inject prior flow MD documents as context
						let flowContextBlock = '';
						if ((task.role === 'coding' || task.role === 'coder') && workspaceFolderPath) {
							const flowMdContent = loadFlowMdFiles(workspaceFolderPath);
							if (flowMdContent) {
								flowContextBlock = `\n\n## Prior Analysis & Design Documents\n\nThe following documents were created by prior specialist agents (design, search, writing, planning). You MUST read and incorporate their findings, decisions, and specifications into your code implementation.\n${flowMdContent}\n\n---\n`;
								appendText(`📚 **Flow MD コンテキスト読み込み完了** (DESIGN.md, SEARCH.md, WRITING.md, PLANNING.md)\n\n`);
							}
						}
						...
						if (result.error) {
							...
							// Save flow result as MD file + trigger review for non-coding roles
							if (FLOW_ROLE_TO_FILENAME[task.role.toLowerCase()]) {
								...
								if (mdPath && i < tasks.length - 1) {
									const mdFilename = FLOW_ROLE_TO_FILENAME[task.role.toLowerCase()];
									const mdContent = fs.readFileSync(mdPath, 'utf-8');
									appendText(`\n📝 **${mdFilename}** に保存しました\n`);
									appendText(`\`${mdPath}\`\n\n`);

									// Save state for resume after review
									saveOrchestrationState(workspaceFolderPath, {
										sessionId, projectId: pendingState.projectId, prompt: originalPrompt,
										tasks, completedTaskIndex: i,
										endpointBase: pendingState.endpointBase, leaderModelName: pendingState.leaderModelName,
										divisionMode: pendingState.divisionMode, selectedModel: pendingState.selectedModel,
									});

									// Signal flow review to browser (stops the agent loop)
									onFinalMessage({
									...
									return; // Pause — wait for user review
								}
							}
						}
					}
			...
			if (result.error) {
				...
				if (workspaceFolderPath) {
					...
					if (savedFiles.length > 0) {
						...
						// Send command operations to renderer to execute in terminal
						if (commands.length > 0 && onCommandRun) {
							onCommandRun(commands);
						}
					}
			...
			if (result.error) {
				...
				if (workspaceFolderPath && FLOW_ROLE_TO_FILENAME[task.role.toLowerCase()]) {
					...
					if (mdPath && i < tasks.length - 1) {
						const mdFilename = FLOW_ROLE_TO_FILENAME[task.role.toLowerCase()];
						const mdContent = fs.readFileSync(mdPath, 'utf-8');
						appendText(`📝 **${mdFilename}** に保存しました\n`);
						appendText(`\`${mdPath}\`\n\n`);

						// Save orchestration state for resume after review
						saveOrchestrationState(workspaceFolderPath, {
						...
						// Signal flow review to browser (pauses the agent loop)
						onFinalMessage({
							fullText, fullReasoning: '', anthropicReasoning: null,
							flowReview: {
								flowRole: task.role,
