...
export const defaultModelsOfProvider = {
	...
	awsBedrock: [],
	liteLLM: [],
	divisionAPI: [ // Division API - multi-agent orchestration (synced from /api/models)
...
// Claude 4.5 以降（claude-4-5-*, claude-4-6-*, claude-4-7-* など）は
// 旧 API の `thinking: { type: 'enabled', budget_tokens }` を受け付けず、
// 新しい `thinking: { type: 'adaptive' }` + `output_config: { effort }` を要求する。
// 参考: "thinking.type.enabled is not supported for this model. Use thinking.type.adaptive
// and output_config.effort to control thinking behavior."
const isAnthropicAdaptiveThinkingModel = (lower: string): boolean => {
	// 4.5 以上（4-5, 4-6, 4-7, 4-8, 4-9）の各 opus/sonnet/haiku、
	// および将来の 5 系をカバー。
	if (/claude-(opus|sonnet|haiku)-4-[5-9]/.test(lower)) return true
	if (/claude-4-[5-9]-(opus|sonnet|haiku)/.test(lower)) return true
	if (/claude-(opus|sonnet|haiku)-[5-9]/.test(lower)) return true
	if (/claude-[5-9]-(opus|sonnet|haiku)/.test(lower)) return true
	return false
}

const anthropicEffortReasoningCapabilities = {
	supportsReasoning: true,
	canTurnOffReasoning: true,
	canIOReasoning: true,
...
const anthropicSettings: VoidStaticProviderInfo = {
		...
		input: {
			includeInPayload: (reasoningInfo) => {
				...
				if (reasoningInfo.type === 'budget_slider_value') {
					// 旧 API 形式（Claude 3.7 / 4.0 系）。
					return { thinking: { type: 'enabled', budget_tokens: reasoningInfo.reasoningBudget } }
				}
				if (reasoningInfo.type === 'effort_slider_value') {
					// 新 API 形式（Claude 4.5+ の adaptive thinking）。
					return {
						thinking: { type: 'adaptive' },
						output_config: { effort: reasoningInfo.reasoningEffort },
					}
				}
				return null
			}
	...
	modelOptionsFallback: (modelName) => {
		...
		// Claude 4.5+ は adaptive thinking を要求するので、reasoning 設定を effort slider で上書き。
		if (isAnthropicAdaptiveThinkingModel(lower)) {
			const base = lower.includes('opus')
				? anthropicModelOptions['claude-opus-4-20250514']
				: anthropicModelOptions['claude-sonnet-4-20250514']
			return {
				modelName,
				recognizedModelName: modelName,
				...base,
				reasoningCapabilities: anthropicEffortReasoningCapabilities,
			}
		}
